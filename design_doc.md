...
**Current implementation snapshot:** Implementation Milestone 9 adds selected-profile processing-job history and verified cleanup of retained terminal job folders. The application preserves each job's `Completed`, `Cancelled`, `Failed`, or `Interrupted` outcome and audit row while permitting explicit deletion of only its freshly revalidated direct child beneath the selected profile's `Processing` root. Active, stale, missing, mismatched, traversal, sibling, root, reparse-point, marker-invalid, or unresolved-promotion targets are refused without being marked cleaned. Cleanup never removes registered media, prepared bundles, persistent results, profile data, sibling jobs, or external staging and never changes readiness, review, media quality, model applicability, or query state. The frozen label-blind Milestone 8 objective-audio observations remain immutable DB-only arithmetic facts, recording dependency groups remain curation metadata rather than an independent sample count, prepared-media review remains available, **Query Profile** remains unavailable, and media quality and model applicability remain `NotAssessed`. This implementation-slice numbering is distinct from the longer-term scientific delivery roadmap in Section 14.
...
Every processing run is confined to one unique `Processing` job folder. Cancellation requests cooperative worker shutdown, terminates an unresponsive worker tree after a bounded grace period, disposes streams and child processes, closes every file handle, records `Cancelled`, and leaves the job folder intact and unlocked for later inspection or deletion. No incomplete derivative bundle, feature set, or candidate model is promoted. The current processing-history UI provides **Open Folder** and **Delete Processing Data**. Each action freshly reloads the database row and verifies profile ownership, exact direct-child containment beneath the selected profile's `Processing` root, job identity and marker, and the absence of reparse-point escapes. Deletion additionally requires an explicit terminal job state, user confirmation, and no unresolved promotion journal.

Verified processing-folder cleanup deletes only the selected job directory and records a nullable cleanup timestamp on the unchanged terminal audit row. A missing folder is not silently treated as cleaned. Active, stale, locked, mismatched, traversal, root, sibling, reparse-point, or unresolved-journal targets fail closed and remain inspectable or retryable. Cleanup does not delete promoted media or derivative bundles, persistent job results, profile metadata, other jobs, feature/model/export/report directories, or external download staging, and it does not implement consent withdrawal, permanent subject-data deletion, secure erasure, or media repair.
...
Cancelling a processing job stops the full worker tree, closes every stream and file handle, records the cancellation, and leaves its unique processing folder intact and unlocked. No partial artifact or model is promoted. The processing-history UI exposes **Open Folder** and **Delete Processing Data** for the selected profile. Folder actions always re-read the job and revalidate the exact filesystem boundary. Cleanup is terminal-only, confirmation-gated, refuses unresolved promotion journals, deletes only that job folder, and retains the job outcome with a cleanup timestamp for restart-visible audit.
